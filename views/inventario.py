...
class InventarioView(ft.Container):
    def __init__(self, page: ft.Page):
        ...
        self.page_ref = page
        self.controller = InventarioController(self)
    ...
    def build_ui(self):
        ...
        self.table_alerts = ft.DataTable(
            columns=[
        ...
        self.search_items = ft.TextField(
        ...
        self.config_min_stock_val = ft.Text("10", color="white", weight=ft.FontWeight.BOLD)
        min_stock_container = ft.Container(
            content=ft.Row([
                ft.Text("Limite mínimo de stock", size=14, color=TEXT_PRIMARY),
                ft.Container(
                    content=self.config_min_stock_val,
        ...
        left_panel = ft.Container(
            ...
            content=ft.Column([
                ...
                ft.Container(
                    content=ft.Column([self.table_alerts], scroll=ft.ScrollMode.AUTO),
                    bgcolor=SURFACE_COLOR, 
        ...
        right_panel = ft.Container(
            expand=True,
            content=ft.Column([
                ft.Row([self.search_items]),
                ft.Container(
                    content=ft.Column([self.table_items], scroll=ft.ScrollMode.AUTO),
                    bgcolor=SURFACE_COLOR, 
                    border_radius=10, 
                    padding=10, 
                    expand=True,
        ...
        header = ft.Row([
            ...
            ft.Button(
                ...
                bgcolor=BACKGROUND_COLOR, 
                color=TEXT_PRIMARY,
                on_click=lambda _: self.open_item_dialog(is_edit=False),
    ...
    def load_data(self):
        # Fetch items from controller
        products = self.controller.get_items(self.search_items.value)
                ...
                ft.DataRow(cells=[
                    ...
                    ft.DataCell(ft.Row([
                        ft.IconButton(
                            ...
                            on_click=lambda _, pid=p.id: self.delete_item(pid)
                        ), 
                        ft.IconButton(
                            ft.Icons.SETTINGS_OUTLINED, 
                            icon_color=SECONDARY_COLOR,
                            on_click=lambda _, prod=p: self.open_item_dialog(is_edit=True, item_data=prod)
                        )
                    ]))
                ])
            )
        ...
        self.table_alerts.rows.clear()
        for a in alerts:
            self.table_alerts.rows.append(
                ft.DataRow(cells=[
    ...
    def delete_item(self, product_id):
        def confirm_delete(e):
            success = self.controller.delete_item(product_id)
