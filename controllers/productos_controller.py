...
class ProductosController:
    ...
    def register_product(self, name, description, price_buy, price_sell, stock, stock_min, category, status):
        if not name or price_sell is None or price_sell == "":
            return False, "El nombre y precio de venta son obligatorios"
        ...
        success = Product.create(name, description, p_buy, p_sell, st, st_min, category, 1 if status else 0)
        if success:
    ...
    def update_product(self, product_id, name, description, price_buy, price_sell, stock, stock_min, category, status):
        if not name or price_sell is None or price_sell == "":
            return False, "Nombre y precio de venta son obligatorios"
        ...
        success = Product.update(product_id, name, description, p_buy, p_sell, st, st_min, category, 1 if status else 0)
        if success:
